...
class FairEstimateBreakDownViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        currencyStr = UserDefaults.standard.string(forKey: "currency")!
        ...
        let jsnStr: String = DATA().getFairEstimateData()
        
        if !jsnStr.isEmpty{
            
            let json: JSON = JSON.init(parseJSON: jsnStr)
            let tax_price = json["tax_price"].stringValue;
            let base_price = json["base_price"].stringValue;
            let min_fare = json["min_fare"].stringValue;
            let booking_fee = json["booking_fee"].stringValue;
            let estimated_fare = json["estimated_fare"].stringValue;
            
            self.baseFareLabel.text = "\(UserDefaults.standard.string(forKey: "currency")!) \(base_price)"
            self.minimumFareLabel.text = "\(UserDefaults.standard.string(forKey: "currency")!) \(min_fare)"
            self.bookingFeeLabel.text = "\(UserDefaults.standard.string(forKey: "currency")!) \(booking_fee)"
            self.serviceTaxLabel.text = "\(UserDefaults.standard.string(forKey: "currency")!) \(tax_price)"
            
            self.perMileLabel.text = "\(UserDefaults.standard.string(forKey: "currency")!) \(taxi.price_distance as String)"
            self.perMinuteLabel.text = "\(UserDefaults.standard.string(forKey: "currency")!) \(taxi.price_min as String)"
        ...
        if error != nil {
            ...
            do {
                ...
                if let json : [String:Any] = try JSONSerialization.jsonObject(with: data!, options: .allowFragments) as? [String: Any]{
                    ...
                    API.getFare(distance: distanceValue, duration: durationValue, serviceType: self.serviceType){ json, error in
                        ...
                        if(status){
                            ...
                            let min_fare = json!["estimated_fare_from"].stringValue;
                            let max_fare = json!["estimated_fare_to"].stringValue;
                            
                            self.approximateFareLabel.text = "$" + " " + max_fare + " - " + min_fare
